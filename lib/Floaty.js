...
var Floaty = function (_SplittablePanel) {
    ...
    _createClass(Floaty, [{
        ...
        value: function renderLeafComponent(leafObject, dispatch, refAccumulator) {
            var _this4 = this;
            ...
            switch (leafObject.type) {
                case 'prop-ref':
                    ...
                    break;
                case 'child-ref':
                    result = _react2.default.Children.toArray(this.props.children)[leafObject.index];
                    break;
                case 'component':
            ...
            if (leafObject.state) {
                var _ret3 = function () {
                    var reducers = _this4.props.reducers;
                    var reducerName = leafObject.reducer;

                    var reducer = reducers[reducerName];
                    var props = _extends({}, leafObject.state);
                    if (reducer) {
                        props.dispatch = function (update) {
                            return dispatch((0, _actions.setStateFromReducer)(reducer, update));
                        };
                    }
                    if (_react2.default.isValidElement(result)) {
                        return {
                            v: _react2.default.cloneElement(result, props)
                        };
