    ...
    _createClass(Floaty, [{
        ...
        value: function dragStart(stackObject, index, event, dispatch, draggable) {
            ...
            draggable.on('drag', function (event) {
                var resolution = _this2.resolveDropArea({ x: event.originalEvent.pageX, y: event.originalEvent.pageY });
                var x = resolution.x;
                var y = resolution.y;
                ...
                if (resolved) {
                    _this2.setState({ x: event.originalEvent.pageX, y: event.originalEvent.pageY, targetIndicator: { x: x, y: y, width: width, height: height }, showTargetIndicator: true });
                } else {
                    _this2.setState({ x: event.originalEvent.pageX, y: event.originalEvent.pageY, showTargetIndicator: false });
                }
            });
            draggable.on('dragstop', function (event) {
                ...
                if (resolution.resolved) {
                    resolution.dispatch(_this2.state.floating, _this2.state.floatingName);
                }
                _this2.setState({ floating: null, floatingName: '' });
