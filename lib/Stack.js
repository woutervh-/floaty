...
var Stack = function (_React$Component) {
    ...
    _createClass(Stack, [{
        ...
        value: function resolveDropArea(position) {
            ...
            var box = _DomUtil2.default.elementOffset(headerElement);
            if (_DomUtil2.default.isWithinBox(position, box)) {
                return _extends({}, box, { action: _actions.noOperation, resolved: true });
            } else {
                var active = this.props.active;

                var activeChildElement = _reactDom2.default.findDOMNode(this.refs['stack-item-' + active]);
                var childBox = _DomUtil2.default.elementOffset(activeChildElement);
                if (_DomUtil2.default.isWithinBox(position, childBox)) {
                    return this.refs['stack-item-' + active].resolveDropArea(position);
                } else {
                    return { x: 0, y: 0, width: 0, height: 0, action: _actions.noOperation, resolved: false };
                }
            }
        }
    }, {
