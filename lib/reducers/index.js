...
function minimizeColumnOrRow(itemId, floatyItems, minimized, type) {
    ...
    if (items.length >= 2) {
        floatyItems[itemId] = { type: self.type, state: self.state, items: items, growValues: growValues };
        return itemId;
    }
...
function minimizeStack(itemId, floatyItems, minimized) {
    ...
    if (items.length >= 1) {
        floatyItems[itemId] = { type: self.type, state: self.state, active: self.active, items: items, titles: titles };
        return itemId;
    }
}
...
function columnOrRow(state, action) {
    switch (action.type) {
        case constants_1.FLOATY_SET_GROW_VALUES:
            return { type: state.type, growValues: action.growValues, items: state.items };
        default:
            return state;
    }
}
    ...
    switch (action.type) {
        case constants_1.FLOATY_REMOVE_TAB: {
            ...
            if (state.active !== undefined) {
                ...
                return { type: state.type, active: active, items: items, titles: titles };
            }
            else {
                return { type: state.type, active: state.active, items: items, titles: titles };
            }
        }
        ...
        case constants_1.FLOATY_ADD_TAB: {
            ...
            return { type: state.type, active: items.length - 1, items: items, titles: titles };
        }
        case constants_1.FLOATY_SET_ACTIVE_TAB:
            return { type: state.type, active: action.index, items: state.items, titles: state.titles };
        default:
            return state;
    }
}
...
function floatyLayout(state, action) {
    ...
    switch (action.type) {
        case constants_1.FLOATY_START_FLOATING:
            return { item: state.item, floatingItem: action.item, floatingTitle: action.title };
        case constants_1.FLOATY_STOP_FLOATING:
            return { item: state.item, floatingItem: null, floatingTitle: null };
        case constants_1.FLOATY_SET_LAYOUT:
            return { item: action.itemId, floatingItem: state.floatingItem, floatingTitle: state.floatingTitle };
        default:
            return state;
    }
}
