...
function minimizeColumnOrRow(itemId, floatyItems, minimized, type) {
    ...
    for (var i = 0; i < self.items.length; i++) {
        ...
        if (childItem) {
            ...
            if ((0, _references.isReference)(childItem) && floatyItems[childItem].type === type) {
                var growValuesSum = 0;
...
function minimize(item, floatyItems, minimized) {
    if ((0, _references.isReference)(item)) {
        ...
        case _constants.FLOATY_TRANSFORM_INTO_ROW:
            {
                ...
                var _ref = [(0, _references.generateIdentifier)(), (0, _references.generateIdentifier)()];
                var id1 = _ref[0];
                var id2 = _ref[1];
...
function floaty() {
    ...
    var next = _extends({}, state, { entities: _extends({}, entities, { floatyItems: _extends({}, floatyItems(items, action)), floatyLayouts: _extends({}, floatyLayouts(layouts, action)) }) });
    var minimized = {};
    Object.keys(next.entities.floatyLayouts).forEach(function (key) {
        var _next$entities$floaty = next.entities.floatyLayouts[key];
        var item = _next$entities$floaty.item;
        var floatingItem = _next$entities$floaty.floatingItem;
