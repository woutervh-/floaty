...
function split(element, position, id, dispatch) {
    ...
    if (DomUtil.isWithinBox(position, leftBox)) {
        ...
        return Object.assign({}, box, { width: box.width / 2, execute: (item, title) => dispatch(actions_1.transformIntoRow(id, { type: 'stack', items: [item], titles: [title] }, true)), resolved: true });
    }
    if (DomUtil.isWithinBox(position, rightBox)) {
        // Make row here: floating content to the right, original content to the left
        return Object.assign({}, box, { x: box.x + box.width / 2, width: box.width / 2, execute: (item, title) => dispatch(actions_1.transformIntoRow(id, { type: 'stack', items: [item], titles: [title] }, false)), resolved: true });
    }
    if (DomUtil.isWithinBox(position, topBox)) {
        return Object.assign({}, box, { height: box.height / 2, execute: (item, title) => dispatch(actions_1.transformIntoColumn(id, { type: 'stack', items: [item], titles: [title] }, true)), resolved: true });
    }
    if (DomUtil.isWithinBox(position, bottomBox)) {
        return Object.assign({}, box, { y: box.y + box.height / 2, height: box.height / 2, execute: (item, title) => dispatch(actions_1.transformIntoColumn(id, { type: 'stack', items: [item], titles: [title] }, false)), resolved: true });
    }
